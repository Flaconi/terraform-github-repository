...
| <a name="input_bot_secrets"></a> [bot\_secrets](#input\_bot\_secrets) | Repository dependabot secrets. | <pre>map(object({<br/>    value           = optional(string)<br/>    value_encrypted = optional(string)<br/>  }))</pre> | `{}` | no |
...
| <a name="input_secrets"></a> [secrets](#input\_secrets) | Repository secrets. | <pre>map(object({<br/>    value           = optional(string)<br/>    value_encrypted = optional(string)<br/>  }))</pre> | `{}` | no |
| <a name="input_security_and_analysis"></a> [security\_and\_analysis](#input\_security\_and\_analysis) | Repository security and analysis settings. Leave as `null` to use the module defaults: disabled for public repositories and unmanaged for non-public repositories. | <pre>object({<br/>    secret_scanning                 = optional(bool, false)<br/>    secret_scanning_push_protection = optional(bool, false)<br/>  })</pre> | `null` | no |
